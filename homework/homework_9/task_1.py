...
class AppleProduct(Produs):
    ...
    def combine_products(self, product):
        if not isinstance(product, AppleProduct):
            return "Produsul nu poate fi conectat deoarece nu este un AppleProduct"
...
class GoogleProduct(AppleProduct):
    def combine_products(self, product):
        if not isinstance(product, GoogleProduct):
            return "Produsul nu poate fi conectat deoarece nu este un GoogleProduct"
        self.produs_conectat = product
        return "Produsul a fost conectat cu succes"
    pass
# CODUL TĂU VINE MAI SUS:
