...
class Sistem:
    ...
    def adauga_utilizator(self, utilizator):
        if isinstance(utilizator, user):
            self.__utilizatori[self.__current_id] = utilizator
            self.__current_id += 1
        else:
            raise ValueError("Obiectul adăugat nu este de tipul corect")
    ...
    def modifica_nivel_acces(self, user_id, new_level):
        if user_id in self.__utilizatori:
            self.__utilizatori[user_id].nivel_acces = new_level
...
class TechSolutionsApp:
    ...
    def __init__(self, versiune_aplicatie="1.0"):
